...
sql = "SELECT num FROM mytable3"
cursor.execute(sql)

       
option_list = [id for id, in cursor]
option_list = [int(x) for x in option_list]




st = time.time()
fp = pd.read_sql(query2, connection)
cp = pd.read_sql(query1, connection)
...
def initializeTables():
    ...
    subject_teacher_dict1 = dict(zip(cp['Faculty_id'], cp['Course_Code']))
